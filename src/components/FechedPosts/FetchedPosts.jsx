    ...
    return (
        <div className="fetched-posts">
            <div className="group-btn">
                <button onClick={() => filterPostsHandler('title')} className="get-posts">Фильтровать по названию</button>
                <button onClick={() => filterPostsHandler('body')} className="get-posts filter-body">Фильтровать по описанию</button>
            </div>
           ...
           <div className="btn-wrapper">
                {pagesArray.map(p =>
                    <button onClick={() => changeCurrentPage(p)} key={p} className="pagination-btn">{p}</button>
                )}
            </div>
        </div>
    )
